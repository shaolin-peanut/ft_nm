...
bool	init_32()
{
	...
	for (int i = 0; i < elf_header->e_shnum; i++) {
		...
		char	*section_name = (char *)(info.sh_str_tab + section_header->sh_name);

		if (section_header->sh_type == SHT_SYMTAB) {
			info.sym_tab = (void *) info.m_elf + section_header->sh_offset;
		...
		if (ft_strncmp(".strtab", section_name, 7) == 0)
			info.sym_str_tab = (void *) info.m_elf + section_header->sh_offset;
	}
	return true;
}
